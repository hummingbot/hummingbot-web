...
<Warning> **The **Secret Private Key** is shown only once. Store it securely before closing the window. If you lose it, you will need to create a new API key.**
...
<Tip> **In the GRVT API documentation, the Trading ID is referred to as `sub_account_id`. Use the Trading ID from the GRVT UI when Hummingbot asks for it.**
## 🔀 Spot Connector
