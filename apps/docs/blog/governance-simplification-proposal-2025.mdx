...
From the start, the Hummingbot Foundation's goal has been to create a sustainable open source project that earns revenues from exchanges and pays community maintainers to build and maintain connectors to their platforms, as outlined in the original [Hummingbot Foundation whitepaper](../../../about/whitepaper).
...
2. **Bounties management services**: The Foundation has expanded its services to help new exchanges manage connector development and maintenance via [bounties](../../../bounties/index)
3. **Platform maturation**: Hummingbot has migrated from a CLI client to an open source ecosystem for algorithmic trading, with [Hummingbot API](../../../hummingbot-api/index) as the command center running on a cloud server and [Condor](../../../condor/index) Telegram bot as the primary client. This makes Hummingbot's entire suite of exchanges and bot strategies more accessible than ever.
...
- **Clarifies Board role**: Currently, Board responsibilities are [unclear](../../../about/board). This change defines a clear, focused responsibility: annual budget approval
