...
Now, install Hummingbot by following the [installation process](/installation). The easiest way to get started is to clone the Github repo and launch the Docker image:
...
This is a simple directional strategy that enters into long positions when the market is oversold (as measured by [RSI](https://en.wikipedia.org/wiki/Relative_strength_index)). Alternatively, if the market is overbought, the bot will enter into a short position. After a position is created, the strategy uses a [PositionExecutor](/documentation/strategies/v2-strategies/executors/positionexecutor) to manage it.
...
* You'll need to use a different exchange (like Binance Futures) other than Vega for `candles_exchange`, since Vega does not currently support [Candles](/documentation/strategies/v2-strategies/candles) yet.
