...
- **[Installation & Setup](installation)**: Complete installation guide for source and Docker
- **[Configuration](configuration)**: How to configure chains, connectors, and settings
- **[Commands](commands)**: Comprehensive reference of all Gateway-related Hummingbot commands
- **[Chains](chains)**: Detailed information about supported chains
- **[DEX Connectors](connectors)**: Guide to all supported DEX in Gateway
- **[Strategies & Scripts](strategies)**: Using Gateway with Hummingbot strategies
...
For detailed implementation guides and examples for each schema, see [DEX Connectors](connectors).

## Installation

Gateway can be installed alongside [Hummingbot](https://github.com/hummingbot/hummingbot) to enable trading on AMM DEXs, or as a standalone API server. For detailed installation instructions, see [Installation & Setup](installation).
...
Hummingbot Foundation maintains certain reference connectors as the standard and utilizes a community-based maintenance process. We assign [Bounties](../bounties/index) to community developers to upgrade and fix bugs for each exchange's connectors in the codebase.

The community can propose changes to connector maintenance, bounty allocations, and connector removals through [Hummingbot Governance Proposals (HGP)](../about/proposals). This process also determines which blockchains and networks that Gateway supports.
...
- **Build new connectors**: See [DEX Connectors](../connectors/gateway-connectors/index) for implementation guide
...
* [Hummingbot Gateway V2 Architecture - Part 1](../blog/posts/hummingbot-gateway-architecture-part-1/index)
* [Hummingbot Gateway V2 Architecture - Part 2](../blog/posts/hummingbot-gateway-architecture-part-2/index)
