...
Some releases include changes to configuration file schemas. When this happens, existing configuration files are incompatible and must be removed before updating. Check the [release notes](../release-notes/index) for your target version to see if configuration changes are required.
