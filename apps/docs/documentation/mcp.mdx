...
- **[MCP Server](./index)** — A running server that implements the [Model Context Protocol](https://modelcontextprotocol.io), letting Claude, Gemini, and ChatGPT control your trading infrastructure via natural language
- **[Skills](./skills)** — Self-contained agent capabilities installable via `npx`, providing commands and scripts for any AI agent framework including [OpenClaw](https://openclaw.ai)

---

# Hummingbot MCP Server

The Hummingbot MCP Server is an interface to [Hummingbot API](../hummingbot-api/index) that enables AI assistants like Claude, Gemini, and ChatGPT to control your trading infrastructure using natural language.
...
1. **[Install the MCP Server](./installation)** - Set up and connect to your AI assistant
2. **[Explore Available Tools](./tools)** - Learn about the trading and portfolio management tools
3. **[Browse Skills](./skills)** - Install agent skills for OpenClaw, Claude Code, and other frameworks
...
See the [Installation Guide](./installation) for setup instructions for each assistant.
