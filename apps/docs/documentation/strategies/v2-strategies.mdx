---
title: "Architecture"
description: "Understanding the three core components and how they relate is key to building effective strategies:"
---
...
[**Executors**](/documentation/strategies/v2-strategies/executors) automate a discrete trading workflow — placing, managing, and closing orders according to predefined logic. They are designed to **start and finish**: each executor has a lifecycle (active → closed/failed) and handles its own order management internally.
...
[**Scripts**](/documentation/strategies/scripts) and [**Controllers**](/documentation/strategies/v2-strategies/controllers) are **long-running processes** inside the Hummingbot client that run continuously until stopped. They are suited for strategies that need to monitor markets, adapt over time, or manage multiple executors in parallel.
...
[**Market Data Provider**](/documentation/strategies/v2-strategies/data): Single point of access to exchange market data — historical OHLCV [Candles](/documentation/strategies/v2-strategies/candles), order book data, and trades. Used by both scripts and controllers to make trading decisions.
...
Check out [Walkthrough - Script](/documentation/strategies/v2-strategies/walkthrough) and [Walkthrough - Controller](/documentation/strategies/v2-strategies/walkthrough-controller) to learn how to create strategies.
