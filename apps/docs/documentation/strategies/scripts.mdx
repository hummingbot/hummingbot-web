...
For **production-grade, multi-pair, or long-running strategies**, consider using [Controllers](../v2-strategies/controllers/index) via the `v2_with_controllers.py` script instead.
...
We welcome new sample script contributions from users! To submit a contribution, please follow the [Contribution Guidelines](../../community/contributions).
...
[![](/images/strategies/v2-strategies/diagrams/14.png)](../v2-strategies/diagrams/14.png)
...
This script fetches data from the Market Data Provider and manages how each Executor behaves. Optionally, it can load a Controller to manage the stategy logic instead of defining it in within the script. Go through the [Walkthrough](../v2-strategies/walkthrough) to learn how it works. 

See [Sample Scripts](../v2-strategies/examples/index) for more examples of StrategyV2-compatible scripts.
