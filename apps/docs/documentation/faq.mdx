...
As we wrote in the original [Hummingbot whitepaper](https://hummingbot.org/blog/hummingbot-whitepaper), market making is an important function critical to organic, efficient markets that should be decentralized to prevent the concentration risk that exists in traditional finance.
...
See [What is market making?](https://hummingbot.org/blog/what-is-market-making) for more information.
...
[Hummingbot Gateway](/documentation/gateway) is API middleware that helps Hummingbot clients interact with decentralized exchanges (DEXs) on various blockchain networks. It:

* Standardizes DEX API endpoints
* Manages interactions with node providers, and
* Utilizes Javascript-based DEX SDKs

Similar to Hummingbot client, Gateway is open source under the Apache 2.0 license. Community developers can contribute DEX and blockchain connectors to the Gateway codebase via [Hummingbot Governance Proposals (HGP)](https://hummingbot.org/about/proposals/).
...
If you just want to get Gateway up and running alongside Hummingbot, following the [Install with Docker](/documentation/gateway/installation) process is the easiest method.

Afterwards, follow the instructions at [Using Gateway with Hummingbot](/documentation/gateway/installation).
...
* [Comparing liquidity mining options in DeFi vs. Hummingbot](https://hummingbot.org/blog/liquidity-mining-in-hummingbot-vs-defi): This CoinAlpha blog post explains how liquidity mining is similar in DeFi and CeFi
...
Check out the [`amm-arb`](https://hummingbot.org/strategies/v1-strategies/amm-arbitrage/) or amm-v3-lp strategies.
...
The Foundation plans to distribute the remaining tokens to Hummingbot users through [governance-approved proposals](https://hummingbot.org/about/proposals/). The goal is to distribute tokens to developers who contribute improvements to the codebase, and users of the Hummingbot software on connected exchanges and market making platforms.

See [Hummingbot Governance Proposals](https://hummingbot.org/about/proposals/) for more information on the categories of HBOT grants.
...
Other than those listed in the [HBOT announcement](https://hummingbot.org/blog/introducing-the-hummingbot-governance-token-hbot), there are no other eligible HBOT recipients.
