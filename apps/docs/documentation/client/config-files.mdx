---
title: "Config Files"
description: "A config file allows you to define the parameters used in a YAML file. Later, you can modify the values of this file, share it with others, and and im"
---
...
Starting in v1.24.0, [Scripts](../strategies/scripts/index) can define a `ScriptConfig` class that defines configuration parameters that users can store in a YAML file.
...
![](/images/client/start-script-config.png)

## Controller config files

The StrategyV2 framework abstracts strategy logic into [Controllers](../strategies/v2-strategies/controllers/index). Each controller defines the config parameters that it accepts.
...
See the [Controller Walkthrough](../strategies/v2-strategies/walkthrough-controller) for the full flow (controller configs plus loader).


## Strategy V1 config files

The original Hummingbot [V1 strategies](../strategies/v1-strategies/index) also allowed users to define config files.

### Creating Strategy V1 config files

Run `create` command without the `--v2-config` flag to create a [Strategy V1](../strategies/v1-strategies/index) file. The autocomplete command will display a list of the available V1 strategies, each one a folder in the [/hummingbot/strategy](https://github.com/hummingbot/hummingbot/tree/master/hummingbot/strategy) folder.
...
![](/images/assets/img/create-file-name.png)
...
![](/images/assets/img/import-command.png)

You can also skip the prompt by running `import [file_name]` command.

![](/images/assets/img/import-file-name.png)
