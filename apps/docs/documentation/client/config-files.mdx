...
Starting in v1.24.0, [Scripts](/documentation/strategies/scripts) can define a `ScriptConfig` class that defines configuration parameters that users can store in a YAML file.
...
The StrategyV2 framework abstracts strategy logic into [Controllers](/documentation/strategies/v2-strategies/controllers). Each controller defines the config parameters that it accepts.
...
See the [Controller Walkthrough](/documentation/strategies/v2-strategies/walkthrough-controller) for the full flow (controller configs plus loader).

## Strategy V1 config files

The original Hummingbot [V1 strategies](https://hummingbot.org/strategies/v1-strategies/) also allowed users to define config files.

### Creating Strategy V1 config files

Run `create` command without the `--v2-config` flag to create a [Strategy V1](https://hummingbot.org/strategies/v1-strategies/) file. The autocomplete command will display a list of the available V1 strategies, each one a folder in the [/hummingbot/strategy](https://github.com/hummingbot/hummingbot/tree/master/hummingbot/strategy) folder.
