...
The config file must exist in `conf/scripts/`. It must include `script_file_name` pointing at the script to run (for example `simple_pmm.py`). Controller-based runs use the `v2_with_controllers` loader; see [Controller Walkthrough](/documentation/strategies/v2-strategies/walkthrough-controller).
