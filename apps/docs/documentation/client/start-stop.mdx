---
title: "Start Strategies and Scripts"
description: "Before starting a V2 strategy or controller, you need to create a config file using the create command."
---
...
The config file must exist in `conf/scripts/`. It must include `script_file_name` pointing at the script to run (for example `simple_pmm.py`). Controller-based runs use the `v2_with_controllers` loader; see [Controller Walkthrough](../strategies/v2-strategies/walkthrough-controller).

![](/images/assets/img/start-command.gif)
...
![](/images/assets/img/stop-command.png)
